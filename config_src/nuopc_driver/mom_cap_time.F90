...
!! we make sure to advance it properly based on the ring interval.
  ...
  subroutine AlarmInit( clock, alarm, option, &
    ...
    !-------------------------------------------------------------------------------
!     write(0,*)' in alarminit option=',trim(option),' opt_n=',opt_n
    ...
    if (trim(option) == optNSteps    .or. trim(option) == optNStep   .or. &
       ...
       if (.not. present(opt_n)) then
          call ESMF_LogSetError(ESMF_RC_ARG_BAD, msg=subname//trim(option)//' requires opt_n', &
                                line=__LINE__, file=__FILE__, rcToReturn=rc)
          return
       end if
       if (opt_n <= 0) then
          call ESMF_LogSetError(ESMF_RC_ARG_BAD, msg=subname//trim(option)//' invalid opt_n', &
                                line=__LINE__, file=__FILE__, rcToReturn=rc)
          return
       end if
!     write(0,*)' in alarminit inside if loop ',trim(option),' opt_n=',opt_n
    endif
...
!     write(0,*)' in alarminit before select case option=',trim(option),' opt_n=',opt_n
    ...
    case (optDate)
       if (.not. present(opt_ymd)) then
          call ESMF_LogSetError(ESMF_RC_ARG_BAD, msg=subname//trim(option)//' requires opt_ymd', &
                                line=__LINE__, file=__FILE__, rcToReturn=rc)
          return
       end if
       if (lymd < 0 .or. ltod < 0) then
          call ESMF_LogSetError(ESMF_RC_ARG_BAD, msg=subname//trim(option)//'opt_ymd, opt_tod invalid', &
                                line=__LINE__, file=__FILE__, rcToReturn=rc)
          return
       end if
       call ESMF_TimeIntervalSet(AlarmInterval, yy=9999, rc=rc)
    ...
    case default
       call ESMF_LogSetError(ESMF_RC_ARG_BAD, msg=subname//' unknown option: '//trim(option), &
                             line=__LINE__, file=__FILE__, rcToReturn=rc)
       return
