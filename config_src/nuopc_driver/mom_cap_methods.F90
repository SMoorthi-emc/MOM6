...
subroutine mom_import(ocean_public, ocean_grid, importState, ice_ocean_boundary, runtype, rc)
  ...
  if (do_import) then
     ...
     !----
     allocate (taux(isc:iec,jsc:jec), tauy(isc:iec,jsc:jec))
...
!$omp parallel do default(shared) private(i,ig,j,jg)
     do j = jsc, jec
...
!$omp parallel do default(shared) private(i,ig,j,jg)
  ...
  allocate(ocz_rot(isc:iec, jsc:jec), ocm_rot(isc:iec, jsc:jec))

!$omp parallel do default(shared) private(i,ig,j,jg,ocz,ocm)
  do j = jsc, jec
     jg = j + ocean_grid%jsc - jsc
     do i = isc, iec
        ig = i + ocean_grid%isc - isc
        ocz          = ocean_public%u_surf(i,j)
        ocm          = ocean_public%v_surf(i,j)
        ocz_rot(i,j) = ocean_grid%cos_rot(ig,jg)*ocz + ocean_grid%sin_rot(ig,jg)*ocm
        ocm_rot(i,j) = ocean_grid%cos_rot(ig,jg)*ocm - ocean_grid%sin_rot(ig,jg)*ocz
     enddo
  enddo
...
!$omp parallel do default(shared) private(i,iloc,j,jloc)
...
!$omp             private(i, iglob, j, jglob, slp_L, slp_R, slp_C, u_min, u_max, slope)
  do jglob = jsc, jec
...
!$omp parallel do default(shared) private(i,i1,ig,j,j1,jg)
        do j = jsc, jec
           j1 = j + lbnd2 - jsc
           jg = j + ocean_grid%jsc - jsc
           do i = isc, iec
