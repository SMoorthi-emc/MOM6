...
character(len=128)   :: restart_prefix = 'MOM.res'
...
subroutine InitializeP0(gcomp, importState, exportState, clock, rc)
  ...
  call NUOPC_CompAttributeGet(gcomp, name="DumpFields_OCN", value=value, &
                              isPresent=isPresent, isSet=isSet, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  if (isPresent .and. isSet) write_diagnostics = (trim(value)=="true")

  restart_prefix = 'MOM.res'
  call NUOPC_CompAttributeGet(gcomp, name="Restart_Prefix", value=value, &
                              isPresent=isPresent, isSet=isSet, rc=rc)
  if (ChkErr(rc,__LINE__,u_FILE_u)) return
  if (isPresent .and. isSet) restart_prefix = trim(value)
...
!--------------------------------
  ...
  restartfile = ""
  if (runtype == "initial") then       ! startup (new run)
...
!$omp parallel do default(shared) private(i,j)
  do j=jsc,jec
    do i=isc,iec
      Ice_ocean_boundary%u_flux(i,j)          = zero
      Ice_ocean_boundary%v_flux(i,j)          = zero
      Ice_ocean_boundary%t_flux(i,j)          = zero
      Ice_ocean_boundary%q_flux(i,j)          = zero
      Ice_ocean_boundary%salt_flux(i,j)       = zero
      Ice_ocean_boundary%lw_flux(i,j)         = zero
      Ice_ocean_boundary%sw_flux_vis_dir(i,j) = zero
      Ice_ocean_boundary%sw_flux_vis_dif(i,j) = zero
      Ice_ocean_boundary%sw_flux_nir_dir(i,j) = zero
      Ice_ocean_boundary%sw_flux_nir_dif(i,j) = zero
      Ice_ocean_boundary%lprec(i,j)           = zero
      Ice_ocean_boundary%fprec(i,j)           = zero
      Ice_ocean_boundary%seaice_melt(i,j)     = zero
      Ice_ocean_boundary%seaice_melt_heat(i,j)= zero
      Ice_ocean_boundary%mi(i,j)              = zero
      Ice_ocean_boundary%p(i,j)               = zero
      Ice_ocean_boundary%lrunoff_hflx(i,j)    = zero
      Ice_ocean_boundary%frunoff_hflx(i,j)    = zero
      Ice_ocean_boundary%lrunoff(i,j)         = zero
      Ice_ocean_boundary%frunoff(i,j)         = zero
    enddo
  enddo
...
subroutine ModelAdvance(gcomp, rc)
  ...
  if (ESMF_AlarmIsRinging(restart_alarm, rc=rc)) then
    ...
    call ESMF_AlarmRingerOff(restart_alarm, rc=rc )   ! turn off the alarm
    if (ChkErr(rc,__LINE__,u_FILE_u)) return
    ...
    if (cesm_coupled) then
      ...
      if (ESMF_AlarmIsRinging(stop_alarm, rc=rc)) then ! write the final restart without a timestamp
        ...
        if (isPresent .and. isSet) then
          ...
          if(restart_n /= 0)then
            ...
            if (isPresent .and. isSet) then
              ...
              call ESMF_LogWrite(subname//" Restart_option = "//restart_option, ESMF_LOGMSG_INFO)
            else
              call ESMF_LogSetError(ESMF_RC_VAL_WRONG, &
